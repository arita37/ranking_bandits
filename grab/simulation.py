...
##########################################################################################
#######  Run with 2 locations  ###########################################################
def run4(cfg:str="config.yaml", name='simul3', dirout='ztmp/exp/', T=1000, nsimul=1, K=2):    
    """ Run Simulation 

         python simulation.py  run4  --cfg config.yaml --name 'simulnew'    --T 10    --dirout ztmp/exp/ --K 2

         python simulation.py  run4  --cfg config.yaml --name 'simulnew'    --T 10000    --dirout ztmp/exp/ --K 2
    ...
    for i in range(nsimul):
        ...
        df, dfstat      = generate_click_data2(cfg= cfg, name=name, T=T, 
                                       dirout= dirouti + f"/data/df_simul_{i}.csv")
        train_grab4(cfg, name, df, dfstat, K=K, dirout=dirouti)


def train_grab4(cfg,name='simul3', df:pd.DataFrame=None, dfstat:pd.DataFrame=None, K=10, dirout="ztmp/"):
    """
    Simulate and test a GRAB-based recommendation system using a provided dataset. 
    Compute the regret at each iteration
