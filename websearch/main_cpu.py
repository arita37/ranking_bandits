...
def train_model(args):
    ...
    print(model.MCAN_CB(config))
    mcan_cb = model.MCAN_CB(config)
    
    mcan_cb.train()
    for param in mcan_cb.parameters():
    ...
    # Loss and Optimizer
    optimizer_ans = torch.optim.Adam(mcan_cb.parameters(), lr=args.lr, betas=(args.beta, 0.999),weight_decay=1e-6)
    if args.lr_sch ==1:    
    ...
    with torch.autograd.set_detect_anomaly(True):
        for epoch in tqdm(range(args.epochs_ext),desc="epoch:"):
            train_iter = data_loader.chunked_data_reader("train")  #-1
            step_in_epoch = 0
